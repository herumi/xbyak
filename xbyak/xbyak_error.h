...
inline const char *ConvertErrorToString(const Error& err)
{
	...
	#define XBYAK_GET_STATUS Xbyak::ERR_NONE
#else
...
// Create a singleton class that contains a static ptr to an object instead of
// initializing it inside the body of instance(). This is a workaround to clang
// with c++98 which produces:
// initializer for thread-local variable must be a constant expression
template <class T>
class Status {
private:
	T status_;
	static THREAD_LOCAL Status* instance_;
...
template <class T>
THREAD_LOCAL Status<T>* Xbyak::Status<T>::instance_ = 0;

	#define XBYAK_SET_STATUS(err) Xbyak::Status<int>::instance().setStatus(err);
	#define XBYAK_GET_STATUS Xbyak::Status<int>::instance().getStatus()
#endif
