...
{
#ifdef XBYAK64
	if (!IsInInt32(x)) XBYAK_SET_STATUS(ERR_OFFSET_IS_TOO_BIG);
#endif
...
public:
	uint8 *alloc(size_t size)
	{
		...
		if (p == MAP_FAILED) XBYAK_SET_STATUS(ERR_CANT_ALLOC);
		assert(p);
	...
	void free(uint8 *p)
	{
		...
		if (i == sizeList_.end()) XBYAK_SET_STATUS(ERR_BAD_PARAMETER);
		if (munmap((void*)i->first, i->second) < 0) XBYAK_SET_STATUS(ERR_MUNMAP);
		sizeList_.erase(i);
	}
};
#endif
...
public:
	...
	{
		if (mask_) XBYAK_SET_STATUS(ERR_OPMASK_IS_ALREADY_SET);
		mask_ = idx;
	}
	void setRounding(int idx)
	{
		if (rounding_) XBYAK_SET_STATUS(ERR_ROUNDING_IS_ALREADY_SET);
		rounding_ = idx;
	}
...
ERR:
	XBYAK_SET_STATUS(ERR_CANT_CONVERT);
}
...
struct RegRip {
	...
	friend const RegRip operator+(const RegRip& r, const Label& label) {
		if (r.label_ || r.isAddr_) XBYAK_SET_STATUS(ERR_BAD_ADDRESSING);
...
public:
	...
	RegExp(const Reg& r, int scale = 1)
		...
		, disp_(0)
	{
		if (!r.isREG(i32e) && !r.is(Reg::XMM|Reg::YMM|Reg::ZMM|Reg::TMM)) XBYAK_SET_STATUS(ERR_BAD_SIZE_OF_REGISTER);
		if (scale == 0) return;
		if (scale != 1 && scale != 2 && scale != 4 && scale != 8) XBYAK_SET_STATUS(ERR_BAD_SCALE);
	...
	void verify() const
	{
		if (base_.getBit() >= 128) XBYAK_SET_STATUS(ERR_BAD_SIZE_OF_REGISTER);
		if (index_.getBit() && index_.getBit() <= 64) {
			if (index_.getIdx() == Operand::ESP) XBYAK_SET_STATUS(ERR_ESP_CANT_BE_INDEX);
			if (base_.getBit() && base_.getBit() != index_.getBit()) XBYAK_SET_STATUS(ERR_BAD_SIZE_OF_REGISTER);
		}
	}
...
{
	if (a.index_.getBit() && b.index_.getBit()) XBYAK_SET_STATUS(ERR_BAD_ADDRESSING);
	RegExp ret = a;
	if (!ret.index_.getBit()) { ret.index_ = b.index_; ret.scale_ = b.scale_; }
	if (b.base_.getBit()) {
		if (ret.base_.getBit()) {
			if (ret.index_.getBit()) XBYAK_SET_STATUS(ERR_BAD_ADDRESSING);
...
protected:
	...
	*/
	void growMemory()
	{
		...
		if (newTop == 0) XBYAK_SET_STATUS(ERR_CANT_ALLOC);
...
public:
	...
	{
		if (maxSize_ > 0 && top_ == 0) XBYAK_SET_STATUS(ERR_CANT_ALLOC);
		if ((type_ == ALLOC_BUF && userPtr != DontSetProtectRWE && useProtect()) && !setProtectMode(PROTECT_RWE, false)) {
			alloc_->free(top_);
			XBYAK_SET_STATUS(ERR_CANT_PROTECT);
		}
	}
	virtual ~CodeArray()
	{
	...
	bool setProtectMode(ProtectMode mode, bool throwException = true)
	{
		...
		if (isOK) return true;
		if (throwException) XBYAK_SET_STATUS(ERR_CANT_PROTECT);
		return false;
	}
	...
	void db(int code)
	{
		if (size_ >= maxSize_) {
			if (type_ == AUTO_GROW) {
				...
				XBYAK_SET_STATUS(ERR_CODE_IS_TOO_BIG);
			}
		}
	...
	{
		if (codeSize > 8) XBYAK_SET_STATUS(ERR_BAD_PARAMETER);
	...
	{
		if (size > maxSize_) XBYAK_SET_STATUS(ERR_OFFSET_IS_TOO_BIG);
		size_ = size;
	}
	void dump() const
	{
	...
	*/
	void rewrite(size_t offset, uint64 disp, size_t size)
	{
		...
		if (size != 1 && size != 2 && size != 4 && size != 8) XBYAK_SET_STATUS(ERR_BAD_PARAMETER);
...
class LabelManager {
	...
	void define_inner(DefList& defList, UndefList& undefList, const T& labelId, size_t addrOffset)
	{
		...
		if (!ret.second) XBYAK_SET_STATUS(ERR_LABEL_IS_REDEFINED);
...
#ifdef XBYAK64
				if (jmp->jmpSize <= 4 && !inner::IsInInt32(disp)) XBYAK_SET_STATUS(ERR_OFFSET_IS_TOO_BIG);
#endif
				if (jmp->jmpSize == 1 && !inner::IsInDisp8((uint32)disp)) XBYAK_SET_STATUS(ERR_LABEL_IS_TOO_FAR);
			}
			if (base_->isAutoGrow()) {
...
public:
	...
	void leaveLocal()
	{
		if (stateList_.size() <= 2) XBYAK_SET_STATUS(ERR_UNDER_LOCAL_LABEL);
		if (hasUndefinedLabel_inner(stateList_.back().undefList)) XBYAK_SET_STATUS(ERR_LABEL_IS_NOT_FOUND);
		stateList_.pop_back();
	}
	void set(CodeArray *base) { base_ = base; }
	void defineSlabel(std::string label)
	{
		if (label == "@b" || label == "@f") XBYAK_SET_STATUS(ERR_BAD_LABEL_STR);
		if (label == "@@") {
	...
	void assign(Label& dst, const Label& src)
	{
		...
		if (i == clabelDefList_.end()) XBYAK_SET_STATUS(ERR_LABEL_ISNOT_SET_BY_L);
	...
	bool getOffset(size_t *offset, std::string& label) const
	{
		...
		if (label == "@b") {
			if (defList.find("@f") != defList.end()) {
				...
				XBYAK_SET_STATUS(ERR_LABEL_IS_NOT_FOUND);
			}
		} else if (label == "@f") {
			if (defList.find("@f") != defList.end()) {
				label = "@b";
...
{
	if (id) XBYAK_SET_STATUS(ERR_LABEL_IS_ALREADY_SET_BY_L);
	id = rhs.id;
	mgr = rhs.mgr;
	if (mgr) mgr->incRefCount(id, this);
	return *this;
...
public:
    ...
    bool is_initialized() const {
        return XBYAK_GET_STATUS == ERR_NONE;
    }
private:
	...
	void rex(const Operand& op1, const Operand& op2 = Operand())
	{
		...
		if (p1->isMEM()) XBYAK_SET_STATUS(ERR_BAD_COMBINATION);
		if (p2->isMEM()) {
	...
	void vex(const Reg& reg, const Reg& base, const Operand *v, int type, int code, bool x = false)
	{
		...
		if ((idx | reg.getIdx() | base.getIdx()) >= 16) XBYAK_SET_STATUS(ERR_BAD_COMBINATION);
	...
	{
		if (!(type & (T_EVEX | T_MUST_EVEX))) XBYAK_SET_STATUS(ERR_EVEX_IS_INVALID);
	...
	void setSIB(const RegExp& e, int reg, int disp8N = 0)
	{
		...
		if (high != 0 && high != 0xFFFFFFFF) XBYAK_SET_STATUS(ERR_OFFSET_IS_TOO_BIG);
#endif
	...
	{
		if (addr.is64bitDisp()) XBYAK_SET_STATUS(ERR_CANT_USE_64BIT_DISP);
		rex(addr, reg);
	...
	{
		if (addr.is64bitDisp()) XBYAK_SET_STATUS(ERR_CANT_USE_64BIT_DISP);
		if (reg.isBit(8)) XBYAK_SET_STATUS(ERR_BAD_SIZE_OF_REGISTER);
		rex(addr, reg);
	...
	{
		if (addr.is64bitDisp()) XBYAK_SET_STATUS(ERR_CANT_USE_64BIT_DISP);
		if (addr.getMode() != Address::M_ModRM) XBYAK_SET_STATUS(ERR_INVALID_MIB_ADDRESS);
		if (BIT == 64 && addr.is32bit()) db(0x67);
	...
	void makeJmp(uint32 disp, LabelType type, uint8 shortCode, uint8 longCode, uint8 longPref)
	{
		...
		if (type != T_NEAR && inner::IsInDisp8(disp - shortJmpSize)) {
			...
			if (type == T_SHORT) XBYAK_SET_STATUS(ERR_LABEL_IS_TOO_FAR);
	...
	{
		if (isAutoGrow()) {
			if (!isNEAR(type)) XBYAK_SET_STATUS(ERR_ONLY_T_NEAR_IS_SUPPORTED_IN_AUTO_GROW);
	...
	{
		if (!permitVisb && addr.isVsib()) XBYAK_SET_STATUS(ERR_BAD_VSIB_ADDRESSING);
		if (addr.getMode() == Address::M_ModRM) {
			...
			if (addr.getLabel()) { // [rip + Label]
				...
				if (addr.getMode() == Address::M_ripAddr) {
					if (isAutoGrow()) XBYAK_SET_STATUS(ERR_INVALID_RIP_IN_AUTO_GROW);
	...
	{
		if (isValid && !isValid(reg, op)) XBYAK_SET_STATUS(ERR_BAD_COMBINATION);
	...
	void opMovXMM(const Operand& op1, const Operand& op2, int code, int pref)
	{
		...
		if (op1.isXMM() && op2.isMEM()) {
			...
			XBYAK_SET_STATUS(ERR_BAD_COMBINATION);
		}
	}
	...
	{
		if (_cl.getIdx() != Operand::CL) XBYAK_SET_STATUS(ERR_BAD_COMBINATION);
	...
	{
		if (_cl && _cl->getIdx() != Operand::CL) XBYAK_SET_STATUS(ERR_BAD_COMBINATION);
	...
	void opRM_I(const Operand& op, uint32 imm, int code, int ext)
	{
		...
		if (op.getBit() < immBit) XBYAK_SET_STATUS(ERR_IMM_IS_TOO_BIG);
	...
	{
		if (op.isMEM() && op.getBit() == 0) XBYAK_SET_STATUS(ERR_MEM_SIZE_IS_NOT_SPECIFIED);
	}
	/*
	...
	*/
	...
	{
		if (op.isBit(32)) XBYAK_SET_STATUS(ERR_BAD_COMBINATION);
		int w = op.isBit(16);
#ifdef XBYAK64
		if (op.isHigh8bit()) XBYAK_SET_STATUS(ERR_BAD_COMBINATION);
#endif
	...
	void opFpuMem(const Address& addr, uint8 m16, uint8 m32, uint8 m64, uint8 ext, uint8 m64ext)
	{
		...
		if (!code) XBYAK_SET_STATUS(ERR_BAD_MEM_SIZE);
	...
	void opFpuFpu(const Fpu& reg1, const Fpu& reg2, uint32 code1, uint32 code2)
	{
		...
		if (!code) XBYAK_SET_STATUS(ERR_BAD_ST_COMBINATION);
		db(uint8(code >> 8));
	...
	void opVex(const Reg& r, const Operand *p1, const Operand& op2, int type, int code, int imm8 = NONE)
	{
		if (op2.isMEM()) {
			...
			if ((type & (T_MUST_EVEX|T_MEM_EVEX)) || r.hasEvex() || (p1 && p1->hasEvex()) || addr.isBroadcast() || addr.getOpmaskIdx()) {
				...
				if (aaa && !(type & T_M_K)) XBYAK_SET_STATUS(ERR_INVALID_OPMASK_WITH_MEMORY);
				bool b = false;
				if (addr.isBroadcast()) {
					if (!(type & (T_B32 | T_B64))) XBYAK_SET_STATUS(ERR_INVALID_BROADCAST);
					b = true;
				}
	...
	void opGpr(const Reg32e& r, const Operand& op1, const Operand& op2, int type, uint8 code, bool isR_R_RM, int imm8 = NONE)
	{
		...
		if (p1->getBit() != bit || (p2->isREG() && p2->getBit() != bit)) XBYAK_SET_STATUS(ERR_BAD_COMBINATION);
	...
	void opAVX_X_X_XM(const Xmm& x1, const Operand& op1, const Operand& op2, int type, int code0, int imm8 = NONE)
	{
		...
		// (x1, x2, op)
		if (!((x1.isXMM() && x2->isXMM()) || ((type & T_YMM) && ((x1.isYMM() && x2->isYMM()) || (x1.isZMM() && x2->isZMM()))))) XBYAK_SET_STATUS(ERR_BAD_COMBINATION);
	...
	{
		if (!op3.isMEM() && (x2.getKind() != op3.getKind())) XBYAK_SET_STATUS(ERR_BAD_COMBINATION);
		opVex(k, &x2, op3, type, code0, imm8);
	}
	// (x, x/m), (y, x/m256), (z, y/m)
	void checkCvt1(const Operand& x, const Operand& op) const
	{
		if (!op.isMEM() && !(x.is(Operand::XMM | Operand::YMM) && op.isXMM()) && !(x.isZMM() && op.isYMM())) XBYAK_SET_STATUS(ERR_BAD_COMBINATION);
	}
	// (x, x/m), (x, y/m256), (y, z/m)
	void checkCvt2(const Xmm& x, const Operand& op) const
	{
		if (!(x.isXMM() && op.is(Operand::XMM | Operand::YMM | Operand::MEM)) && !(x.isYMM() && op.is(Operand::ZMM | Operand::MEM))) XBYAK_SET_STATUS(ERR_BAD_COMBINATION);
	}
	...
	{
		if (!(x1.isXMM() && x2.isXMM() && (op.isREG(i32e) || op.isMEM()))) XBYAK_SET_STATUS(ERR_BAD_SIZE_OF_REGISTER);
		Xmm x(op.getIdx());
	...
	void opSp1(const Reg& reg, const Operand& op, uint8 pref, uint8 code0, uint8 code1)
	{
		...
		if (!is16bit && !(reg.isREG(i32e) && (op.isREG(reg.getBit()) || op.isMEM()))) XBYAK_SET_STATUS(ERR_BAD_COMBINATION);
		if (is16bit) db(0x66);
	...
	{
		const RegExp& regExp = addr.getRegExp();
		if (!regExp.isVsib(128 | 256)) XBYAK_SET_STATUS(ERR_BAD_VSIB_ADDRESSING);
		const int y_vx_y = 0;
		const int y_vy_y = 1;
//		const int x_vy_x = 2;
		...
		if (!x1.isXMM() || isAddrYMM || !x2.isXMM()) {
			...
			if (!isOK) XBYAK_SET_STATUS(ERR_BAD_VSIB_ADDRESSING);
		}
	...
	void checkGather2(const Xmm& x1, const Reg& x2, int mode) const
	{
		...
		XBYAK_SET_STATUS(ERR_BAD_VSIB_ADDRESSING);
	}
	void opGather2(const Xmm& x, const Address& addr, int type, uint8 code, int mode)
	{
		if (x.hasZero()) XBYAK_SET_STATUS(ERR_INVALID_ZERO);
	...
	*/
	...
	{
		if (mode) {
			if (!op.isMEM() && !((op.isXMM() && x.isXMM()) || (op.isXMM() && x.isYMM()) || (op.isYMM() && x.isZMM())))  XBYAK_SET_STATUS(ERR_BAD_COMBINATION);
		} else {
			if (!op.isMEM() && !op.isXMM()) XBYAK_SET_STATUS(ERR_BAD_COMBINATION);
		}
		opVex(x, 0, op, type, code);
	}
	void opGatherFetch(const Address& addr, const Xmm& x, int type, uint8 code, Operand::Kind kind)
	{
		if (addr.hasZero()) XBYAK_SET_STATUS(ERR_INVALID_ZERO);
		if (addr.getRegExp().getIndex().getKind() != kind) XBYAK_SET_STATUS(ERR_BAD_VSIB_ADDRESSING);
		opVex(x, 0, addr, type, code);
	}
	...
	void opAMX(const Tmm& t1, const Address& addr, int type, int code0)
	{
		...
		if (exp.getBase().getBit() == 0 || exp.getIndex().getBit() == 0) XBYAK_SET_STATUS(ERR_NOT_SUPPORTED);
...
public:
	...
	*/
	...
	void mov(const Operand& op, uint64 imm)
	{
		if (op.isREG()) {
			...
			if (immSize <= 4) {
				...
				if (s != 0 && s != -1) XBYAK_SET_STATUS(ERR_IMM_IS_TOO_BIG);
			} else {
				if (!inner::IsInInt32(imm)) XBYAK_SET_STATUS(ERR_IMM_IS_TOO_BIG);
				immSize = 4;
			}
	...
	void pop(const Segment& seg)
	{
		...
		case Segment::cs: XBYAK_SET_STATUS(ERR_BAD_COMBINATION);
	...
	*/
	...
	{
		if (hasUndefinedLabel()) XBYAK_SET_STATUS(ERR_LABEL_IS_NOT_FOUND);
		if (isAutoGrow()) {
			calcJmpAddress();
...
#include "xbyak_mnemonic.h"
	...
	*/
	...
	{
		if (x == 1) return;
		if (x < 1 || (x & (x - 1))) XBYAK_SET_STATUS(ERR_BAD_ALIGN);
