...
CYBOZU_TEST_AUTO(lzcnt)
{
	...
	const uint8_t tbl[] = {
		...
		0xfc, 0x08, 0xf4, 0x02,
		// popcnt
		0x62, 0xec, 0x7d, 0x08, 0x88, 0xc1, 0x62, 0xec, 0x7c, 0x08, 0x88, 0xc1, 0x62, 0xec, 0xfc, 0x08,
		0x88, 0xc1, 0x62, 0xec, 0xfc, 0x0c, 0x88, 0xc1, 0x62, 0xf4, 0xfc, 0x0c, 0x88, 0xc1, 0x62, 0xfc,
		0xfc, 0x08, 0x88, 0x02,
	};
	const size_t n = sizeof(tbl);
	CYBOZU_TEST_EQUAL(c.getSize(), n);
...
CYBOZU_TEST_AUTO(aeskl)
{
	...
	struct CodeErr : Xbyak::CodeGenerator {
		CodeErr()
		{
	...
	const uint8_t tbl[] = {
		...
		// aesdecwide256kl
		0xf3, 0x0f, 0x38, 0xd8, 0x5c, 0x88, 0x12,
		// aesenc128kl
...
CYBOZU_TEST_AUTO(encodekey)
{
	...
	const uint8_t tbl[] = {
		...
		0xf3, 0x41, 0x0f, 0x38, 0xfa, 0xc0,
		0xf3, 0x44, 0x0f, 0x38, 0xfa, 0xc3,
		// encodekey256
		0xf3, 0x0f, 0x38, 0xfb, 0xc3,
		0xf3, 0x41, 0x0f, 0x38, 0xfb, 0xc0,
		0xf3, 0x44, 0x0f, 0x38, 0xfb, 0xc3,
	};
	const size_t n = sizeof(tbl);
	CYBOZU_TEST_EQUAL(c.getSize(), n);
	CYBOZU_TEST_EQUAL_ARRAY(c.getCode(), tbl, n);
...
CYBOZU_TEST_AUTO(sha)
{
	struct Code : Xbyak::CodeGenerator {
		Code()
		{
			sha1msg1(xmm15, ptr [rax+rcx*4+0x12]);
			sha1msg2(xmm15, ptr [rax+rcx*4+0x12]);
			sha1nexte(xmm15, ptr [rax+rcx*4+0x12]);
			sha256msg1(xmm15, ptr [rax+rcx*4+0x12]);
			sha256msg2(xmm15, ptr [rax+rcx*4+0x12]);
			sha256rnds2(xmm15, ptr [rax+rcx*4+0x12]);
			sha1rnds4(xmm15, ptr [rax+rcx*4+0x12], 0x23);
		}
	} c;
	struct CodeErr : Xbyak::CodeGenerator {
		CodeErr()
	...
	const uint8_t tbl[] = {
		0x44, 0x0f, 0x38, 0xc9, 0x7c, 0x88, 0x12,
		0x44, 0x0f, 0x38, 0xca, 0x7c, 0x88, 0x12,
		0x44, 0x0f, 0x38, 0xc8, 0x7c, 0x88, 0x12,
		0x44, 0x0f, 0x38, 0xcc, 0x7c, 0x88, 0x12,
		0x44, 0x0f, 0x38, 0xcd, 0x7c, 0x88, 0x12,
		0x44, 0x0f, 0x38, 0xcb, 0x7c, 0x88, 0x12,
		0x44, 0x0f, 0x3a, 0xcc, 0x7c, 0x88, 0x12, 0x23,
	};
	const size_t n = sizeof(tbl);
	CYBOZU_TEST_EQUAL(c.getSize(), n);
	CYBOZU_TEST_EQUAL_ARRAY(c.getCode(), tbl, n);
